function [u_evol, x_evol, J_evol, cost_evol, total_cost] = RHM1(Ny,Nu,r,T)
...
[u_star, ~] = optimise_step1(x,t,Ny,Nu); % optimal u before shop opens (prediction and optimize u)
...
for t = 1:T
    ...
    if (x <= r) && (t<52) % update stock if below threshold
        [u_star, J_star] = optimise_step1(x,t,Ny,Nu); % optimal u for t+1
    else
        u_star = 0; J_star=0;
    end
